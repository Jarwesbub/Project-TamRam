...
public class EnemyClass3AttDef : MonoBehaviour
{
        ...
        if (EnHealth >= 0 && PersistentManagerScript.Instance.EnemyTurn == false)
        {
            ...
            if (PlayerClass == 2) // Super Attack from Class2 -> makes 10 dmg/ poison damage ///Cost-10 MP
            {
                ...
                if (DEXBuff >= EnCon)
                {
                    ...
                    EnHealth -= (DexUltra + DEXBuff) - EnCon;
                    PlayerDamageDone();
                    yield return new WaitForSeconds(DmgCalcTime);
                    DmgDoneTxt.text = " ";
    ...
    IEnumerator EnemyBasicAttack()
    {
            ...
            if (StunEffect == 0) // CHECK CLASS1 SUPER ATTACK STATUS EFFECTS
            {
                ...
                if (WeakenEffect == 5)
                {
                    ...
                    */
                    ...
                    EnIntNerf -= (EnIntNerf / 4);
                    EnIntBuff -= (EnIntBuff / 4);
                    //WeakenEffect -= 1;
                ...
                if (PlayerClass == 1)
                    ...
                    if (PlayerClass == 2)
                    {
                        ...
                        if (EnIntNerf >= CON)
                        {
                            ...
                            Class2AttBuffValue += (EnInt - CON) / 2;
                            PersistentManagerScript.Instance.PlayerHealth = PlayerHealth;
                            PlayerDamageTaken();
                            yield return new WaitForSeconds(DmgCalcTime);
                            DmgTakenTxt.text = " ";
        ...
        if (PlayerDefUP == true)
        {
            ...
            if (PlayerClass == 3)  // Class 3 Get Mana
            {
                if ((INT / 2) >= EnCon)
                {
